...
SYSTEM_PROMPT = (
    ...
    "- НИКОГДА не придумывай значения показателей. Указывай ТОЛЬКО то, что реально написано на бланке.\n"
    "- Если у показателя на бланке указана только норма (референсный диапазон), "
    "а результат отсутствует — в таблице в колонке значения поставь прочерк «—», "
    "укажи норму и статус «Не выполнен». В подробном разборе этот показатель НЕ расписывай, "
    "просто напиши: анализ на этот показатель не проводился."
)
